...
After 195+ documented build sessions, everything runs locally on owned hardware: zero cloud AI dependencies, zero external speech APIs, zero telemetry.

### Signature integrations

- **Lifelike gaze-tracking eyes**: the namesake feature, a high-detail TeensyEyes render (Adafruit Uncanny Eyes lineage) on twin 240×240 round displays, with a DFRobot SEN0626 face-tracking sensor literally driving the gaze so IRIS's eyes follow you around the room. Pupils dilate and the eye style shifts with emotion. A Useful Sensors Person Sensor pans the whole head to track you: deliberately, glacially slow. The eyes used a Person Sensor too until that part was discontinued; its replacement was built and proven in its own public repo, [CyclopsGaze](https://github.com/Maestro8484/CyclopsGaze).
- **Camera vision & recognition**: a Raspberry Pi Camera Module 3 feeds a vision-capable LLM (Mistral Small with Pixtral vision baked in). IRIS plays reciprocal I Spy / Show Me / Make a Face (seeing, reacting in character, and keeping the back-and-forth going for several turns without re-waking) and recognizes specific people, and the family dog, by name from descriptions written into its persona modelfile.
...
## How IRIS grew: the capability timeline

IRIS did not start as a robot. It started as a competent voice assistant nobody wanted to talk to, and it grew from the eyes outward. Read this the way you would read a child's milestones. It also doubles as the feature index: every row links to where that capability lives in the sections below.

| Capability | What it added |
|---|---|
| **Eyes that follow you** *(the genesis)* | The founding feature and the namesake. Animated eyes (Adafruit Uncanny Eyes lineage) that find your face and track you across a room. Not a voice assistant with a face bolted on, but the thing everything else was built outward from. |
| **Her own voice** | Kokoro as the primary voice, Piper as the offline fallback. |
| **Sight** | A camera and a vision model. The eyes already knew *where* you were; this was the first time she could tell *what* something was. |
| **Reflexes** | An intent router in front of the LLM: "stop", volume, sleep, and the time answer instantly instead of waiting on a 24B model. |
| **A face that shows what she means** | Every reply carries an emotion that drives eyes, mouth, and LEDs at the moment she speaks. |
| **Conversational timing** | Streaming speech. First audio in about 1.5 s instead of about 23 s for a full reply. |
| **A mouth that matches the words** | Jaw movement driven by real per-word timing from the voice engine, not a blind open-and-close. |
| **She can be interrupted** | Say "stop" mid-sentence and she stops, recognized on-device in about 200 ms. |
| **A second self, for children** | A separate model, warmth, tempo, and vocabulary for kids. |
| **Play, not description** | I Spy, Rock Paper Scissors, and Guess My Drawing. She uses the camera to hold a secret, take a turn, and be wrong out loud, rather than just narrating a frame. |
| **Sustained conversation** | The mic reopens after every reply. No wake word between turns, up to 25 turns. She holds the floor. |
| **Memory, and knowing when she has none** | She can be asked about earlier conversations. When her record comes up empty she says so plainly instead of inventing an answer. |

One of these was earned the hard way. For a long stretch the camera turned out to have never actually been focusing: capture skipped the autofocus cycle, so the lens sat parked at room distance and a game that reads a hand at close range kept failing, no matter how the prompt was tuned. Finding that took a game IRIS kept losing. The fix was one capture flag, and hand reading went from below chance to usable.
...
| Node | Role |
|---|---|
| **Raspberry Pi 4** (overlayfs, read-only SD) | Orchestrator: wakeword, ReSpeaker 2-Mic HAT audio (GPIO ribbon), intent routing, streaming dispatch, LEDs, Raspberry Pi Camera Module 3, Teensy serial bridge, WebUI (port 5000), cron sleep/wake |
...
| **Teensy 4.1** (USB serial) | Display controller: two GC9A01A round-TFT eyes + ILI9341 TFT mouth (10 expression frames), DFRobot SEN0626 face-tracking sensor on UART for gaze tracking, sleep starfield renderer |
| **Teensy 4.0** (USB serial) | Base controller: pan servo (ServoEasing) + PAJ7620U2 gesture sensor, Useful Sensors Person Sensor on I2C for head tracking |
...
14. **Vision Cal**. Runtime tuning of the eye-tracking sensors with **no firmware rebuild**: live `PS_CFG` values (confidence gate, timing) are sent to the Teensy 4.1 over serial and re-applied from `ps_config.json` on every restart; a sensor health card decodes probe results and face events into ALIVE/ABSENT; and the head-tracking Person Sensor has an indicator-LED toggle giving a one-click liveness proof (the LED lights only if the I2C write lands).
